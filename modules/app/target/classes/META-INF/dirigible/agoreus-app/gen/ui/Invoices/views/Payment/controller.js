...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'agoreus-app.Invoices.Payment.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('agoreus-app.Invoices.Payment.refresh', callback);
		},
		onInvoicesModified: function(callback) {
			on('agoreus-app.Invoices.Invoices.modified', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	function invoicesidOptionsLoad() {
		$http.get(invoicesidOptionsApi)
		.then(function(data) {
			$scope.invoicesidOptions = data.data;
		});
	}
	invoicesidOptionsLoad();
	...
	$scope.invoicesidOptionValue = function(optionKey) {
		for (var i = 0 ; i < $scope.invoicesidOptions.length; i ++) {
			if ($scope.invoicesidOptions[i].Id === optionKey) {
				return $scope.invoicesidOptions[i].Id;
			}
		}
		return null;
	};
